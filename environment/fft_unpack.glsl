...
void main() {
	...
	// Transpose the output coordinate because the FFT pipeline skips the second transpose pass.
   	const ivec3 id_out = ivec3(id.y, id.x, id.z);
	...
	switch (id_local.z) {
		case 0:
			...
			imageStore(displacement_map, id_out, vec4(hx, hy, hz, 0) * sign_shift);
           		break;
		case 1:
