...
func init_gpu(num_cascades: int) -> void:
	...
	var spectrum_compute_shader: RID = context.load_shader('res://assets/ocean_waves/shaders/compute/spectrum_compute.glsl')
	var fft_butterfly_shader: RID = context.load_shader('res://assets/ocean_waves/shaders/compute/fft_butterfly.glsl')
	var spectrum_modulate_shader: RID = context.load_shader('res://assets/ocean_waves/shaders/compute/spectrum_modulate.glsl')
	...
	descriptors[&'downsampled_map'] = context.create_texture(cpu_dims, RenderingDevice.DATA_FORMAT_R16G16B16A16_SFLOAT, RenderingDevice.TEXTURE_USAGE_STORAGE_BIT | RenderingDevice.TEXTURE_USAGE_CAN_COPY_FROM_BIT, num_cascades)
	...
	var spectrum_modulate_set_0: RID = context.create_descriptor_set([descriptors[&'spectrum']], spectrum_modulate_shader, 0)
	var spectrum_modulate_set_1: RID = context.create_descriptor_set([descriptors[&'fft_buffer']], spectrum_modulate_shader, 1)

	var fft_butterfly_set: RID = context.create_descriptor_set([descriptors[&'butterfly_factors']], fft_butterfly_shader, 0)
	var fft_compute_set: RID = context.create_descriptor_set([descriptors[&'butterfly_factors'], descriptors[&'fft_buffer']], fft_compute_shader, 0)
	var transpose_set: RID = context.create_descriptor_set([descriptors[&'butterfly_factors'], descriptors[&'fft_buffer']], transpose_shader, 0)

	var unpack_set_0: RID = context.create_descriptor_set([descriptors[&'displacement_map'], descriptors[&'normal_map']], fft_unpack_shader, 0)
	var unpack_set_1: RID = context.create_descriptor_set([descriptors[&'fft_buffer']], fft_unpack_shader, 1)
	
	var downsample_set: RID = context.create_descriptor_set([descriptors[&'displacement_map'], descriptors[&'downsampled_map']], downsample_shader, 0)
	
	# --- COMPUTE PIPELINE CREATION ---
	pipelines[&'spectrum_compute'] = context.create_pipeline([map_size/16.0, map_size/16.0, 1], [spectrum_set], spectrum_compute_shader)
	pipelines[&'spectrum_modulate'] = context.create_pipeline([map_size/16.0, map_size/16.0, 1], [spectrum_modulate_set_0, spectrum_modulate_set_1], spectrum_modulate_shader)
	pipelines[&'fft_butterfly'] = context.create_pipeline([map_size/2.0/64.0, num_fft_stages, 1], [fft_butterfly_set], fft_butterfly_shader)
	pipelines[&'fft_compute'] = context.create_pipeline([1, map_size, 4], [fft_compute_set], fft_compute_shader)
	pipelines[&'transpose'] = context.create_pipeline([map_size/32.0, map_size/32.0, 4], [transpose_set], transpose_shader)
	pipelines[&'fft_unpack'] = context.create_pipeline([map_size/16.0, map_size/16.0, 1], [unpack_set_0, unpack_set_1], fft_unpack_shader)
	pipelines[&'downsample'] = context.create_pipeline([cpu_map_size/16.0, cpu_map_size/16.0, 1], [downsample_set], downsample_shader)
...
func _update(compute_list: int, cascade_index: int, parameters: Array[WaveCascadeParameters]) -> void:
	...
	pipelines[&'downsample'].call(context, compute_list, RenderingContext.create_push_constant([
		int(cascade_index), 
		float(ratio)
	]))
