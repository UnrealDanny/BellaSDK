...
@export_color_no_alpha var crest_color: Color = Color(0.0, 0.65, 0.85) :
	set(value):
		crest_color = value
		if WATER_MAT: WATER_MAT.set_shader_parameter("crest_color", crest_color)

@export_range(0.0, 2.0) var crest_glow_intensity: float = 0.8 :
	set(value):
		crest_glow_intensity = value
		if WATER_MAT: WATER_MAT.set_shader_parameter("crest_glow_intensity", crest_glow_intensity)

@export_range(0.0, 2.0) var aerated_foam_glow: float = 0.5 :
	set(value):
		aerated_foam_glow = value
		if WATER_MAT: WATER_MAT.set_shader_parameter("aerated_foam_glow", aerated_foam_glow)
...
@export var mesh_quality: MeshQuality = MeshQuality.HIGH :
	set(value):
		mesh_quality = value
		if mesh_quality == MeshQuality.LOW: mesh = WATER_MESH_LOW
		if mesh_quality == MeshQuality.HIGH: mesh = WATER_MESH_HIGH
		if mesh_quality == MeshQuality.HIGH8K: mesh = WATER_MESH_HIGH8K
...
@export var bake_waves_to_res: bool = false :
	set(value):
		if value: bake_waves_to_res_routine() 
		bake_waves_to_res = false

@export var reset_cascades: bool = false :
	set(value):
		if value: force_reset_cascades()
...
func _process(delta: float) -> void:
	...
	if updates_per_second == 0.0 or time >= next_update_time:
		...
		var update_delta: float = delta if updates_per_second == 0.0 else target_update_delta + (time - next_update_time)
...
func _do_texture_readback(idx: int) -> void:
	...
	var img: Image = Image.create_from_data(wave_generator.cpu_map_size, wave_generator.cpu_map_size, false, Image.FORMAT_RGBAH, tex) # <-- CHANGED
