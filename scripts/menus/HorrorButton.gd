...
func _ready() -> void:
	# --- FIX 1: FORCE-DELETE LOCKED BACKGROUND STYLES ---
	flat = true # Tells the button not to draw its default background
	...
	if text_label != null:
		# FIX: We must use manual sizing, NOT anchors, because we animate position in _process
...
func _on_resized() -> void:
	...
	# Send the corrected bounds to the shaders
...
func _process(delta: float) -> void:
	...
	# CRITICAL MATH FIX: Shaders need standard bounds, NOT multiplied by scale!
