		...
		if event is InputEventKey or event is InputEventMouseButton:
			if event.is_pressed():
				...
				if conflicting_action != "" and conflicting_action != action_to_remap:
					...
					get_tree().create_timer(4.0).timeout.connect(func(): %ConflictPanel.hide())
...
func refresh_all_button_labels():
	var container = $ControlsPanel/VBoxContainer
	...
	for child in container.get_children():
		...
		if child is Button and child.has_meta("action"):
			var action_name = child.get_meta("action")
...
func execute_swap(new_event: InputEvent):
