...
func drop() -> void:
	...
	if holder:
		...
		if push_dir.length_squared() < 0.001:
			push_dir = Vector2(-previous_holder.global_transform.basis.z.x, -previous_holder.global_transform.basis.z.z).normalized()
		...
		detach_tween.tween_property(previous_holder, "global_position", target_pos, 0.5)\
